...
foreach ($u in $users) {
    ...
    $upn = "$sam@nektar.com"
    $remote = "$sam@nektar2.mail.onmicrosoft.com"
    ...
    # Select template
    if ($u.Template -eq "SFMBTemplateNFTE") {
        $template = "SFMBTemplateNFTE"
    }
    elseif ($u.Template -eq "SFMBTemplate") {
        $template = "SFMBTemplate"
    }
    else {
    ...
    New-ADUser `
        ...
        -Path "OU=SFMB,OU=San Francisco,OU=Nektar Users,DC=nektar,DC=com"
